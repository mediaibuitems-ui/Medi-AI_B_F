/**
 * write_thesis.js — Medi-AI Final Thesis Generator (Extended v2)
 *
 * STEP 0 FILE ACCESS REPORT (prepended to every run):
 * - Mide-AI - Final.docx: READABLE via Node ZIP parser (extract_docx.js)
 * - write_thesis.js: READ & EXTENDED (this file)
 * - diagrams/: ALL 7 Mermaid .md files READ (0_main_navigation through 6_profile_settings)
 * - Pictures to add in Thesis/: 21 files INVENTORIED (cannot view pixel content;
 *   captions written from filename + cross-referenced Flutter screen code)
 * - Flutter screen .dart files: READ (all 44 screen files inspected)
 * - Backend controllers, MediaidbContext.cs, Program.cs, pubspec.yaml: READ (previous session)
 * - UNREADABLE: None. All required files accessible.
 *
 * PATH: Took Convert-then-Edit approach:
 *   Mide-AI - Final.docx → docx_raw_text.txt (via extract_docx.js)
 *   Final output: Medi-AI - Final (Updated).md (this script)
 */

const fs = require('fs');

// ─────────────────────────────────────────────────────────────
// MERMAID SOURCE BLOCKS (all 7 diagrams, corrected)
// NOTE: diagram 0_main_navigation.md has ERRORS — Faculty shown with AI/Appointment
// access which is factually wrong per codebase. Corrected below.
// NOTE: diagram 2_medicine_manager.md uses wrong route /api/reminders — corrected.
// ─────────────────────────────────────────────────────────────

const DIAG_MAIN_NAVIGATION = `\`\`\`mermaid
flowchart TB
...
const DIAG_SYMPTOM_ANALYSIS = `\`\`\`mermaid
flowchart TB
...
const DIAG_MEDICINE_MANAGER = `\`\`\`mermaid
flowchart TB
...
const DIAG_MEDICINE_ALARMS = `\`\`\`mermaid
flowchart TB
...
const DIAG_FIND_DOCTORS = `\`\`\`mermaid
flowchart TB
...
const DIAG_BOOK_APPOINTMENT = `\`\`\`mermaid
flowchart TB
...
const DIAG_PROFILE_SETTINGS = `\`\`\`mermaid
flowchart TB
...
// ─────────────────────────────────────────────────────────────
// SCREENSHOT CAPTIONS — code-verified from Flutter .dart files
// Cannot view pixel content. Captions written from:
//   1. Filename (primary signal)
//   2. Corresponding Flutter screen file (AppBar title, main widgets)
//   3. Cross-reference with controller data fields
// ─────────────────────────────────────────────────────────────

const SCREENSHOTS = [
  {
    file: 'Splash Screen Medi-Ai.jpg',
    figNum: 24,
    caption: 'Figure 24: Splash Screen — Application Launch View',
    role: 'All Users',
    description: 'Animated splash screen displayed on application launch. Shows the Medi-AI logo/branding with loading animation before routing to the Login Screen. Implemented in `splash_screen.dart`; uses `GetX` to check stored JWT validity and routes to the appropriate role dashboard or Login Screen.',
    screenFile: 'lib/app/modules/auth/splash/splash_screen.dart'
  },
  {
    file: 'Login Screen Medi-Ai.jpg',
    figNum: 25,
    caption: 'Figure 25: Login Screen — Authentication View',
    role: 'All Users',
    description: 'Email and password login form with a gradient background (AppTheme.primary top to bottom). Contains: Email TextField with validation, Password TextField with show/hide toggle, "Login" primary button (calls `POST /api/Auth/login`), "Forgot Password?" text button, "Register" navigation link. Displays "Invalid credentials" error on failure. JWT tokens stored to `flutter_secure_storage` on success.',
    screenFile: 'lib/app/modules/auth/login/login_screen.dart'
  },
  {
    file: 'Create Account Screen Medi-Ai.jpg',
    figNum: 26,
    caption: 'Figure 26: Create Account Screen — User Registration View',
    role: 'All Users (Registration)',
    description: 'Multi-field registration form. Contains: Full Name, Email (validated against @buitms.edu.pk format by UI), Password + Confirm Password, Role dropdown (Student/Faculty/Doctor/Admin), Department, Registration Number, Phone Number. "Register" button calls `POST /api/Auth/register`; on success, navigates to OTP Verification screen. Form validated with GlobalKey<FormState>.',
    screenFile: 'lib/app/modules/auth/register_email/register_email_screen.dart'
  },
  {
    file: 'Forgot passwrod Screen Medi-Ai.jpg',
    figNum: 27,
    caption: 'Figure 27: Forgot Password Screen — Password Reset View',
    role: 'All Users',
    description: 'Password reset request screen. Contains: Email TextField, "Send Reset OTP" button (calls `POST /api/Auth/forgot-password`). On success, displays confirmation message and navigates to OTP verification. Source filename contains typo ("passwrod") — corrected in caption.',
    screenFile: 'lib/app/modules/auth/forgot_password/forgot_password_screen.dart'
  },
  {
    file: 'Student Dashborad Screen Medi-Ai.jpg',
    figNum: 28,
    caption: 'Figure 28: Student Dashboard — Student Role Home View',
    role: 'Student',
    description: 'Primary landing screen for authenticated students. AppBar: "Student Dashboard" title + notification bell badge + hamburger menu drawer. Body sections: (1) Welcome card with student name and current date; (2) Statistics cards: total appointments, pending, completed; (3) Quick Action grid buttons: Book Appointment, AI Symptom Analyzer, Medicine Reminders, My Appointments, Prescription History, Medical History; (4) Upcoming Appointments list with doctor name + date + status chip. Source filename contains typo ("Dashborad") — corrected in caption.',
    screenFile: 'lib/app/modules/student/dashboard/student_dashboard_screen.dart'
  },
  {
    file: 'booking Appointment Screen for Faculty and Student Feature.jpg',
    figNum: 29,
    caption: 'Figure 29: Book Appointment Screen — Student/Faculty View',
    role: 'Student / Faculty',
    description: 'Appointment booking form. AppBar: "Book Appointment" title. Form fields (in ListView with 16px padding): (1) Doctor selection dropdown (loads from `GET /api/Doctors/available`); (2) Date picker (calendar — no past dates allowed); (3) Time slot dropdown (loads from `GET /api/Doctors/{id}/available-slots?date={date}`); (4) Symptoms/reason TextField (multiline); (5) Notes TextField (optional). "Book Appointment" submit button (calls `POST /api/appointments`, rate-limited 5/min). Shows loading indicator while submitting.',
    screenFile: 'lib/app/modules/student/book_appointment/book_appointment_screen.dart'
  },
  {
    file: 'My Appointments Screen for Faculty and Student Feature.jpg',
    figNum: 30,
    caption: 'Figure 30: My Appointments Screen — Student/Faculty View',
    role: 'Student / Faculty',
    description: 'Appointment history and status view. AppBar: "My Appointments". Body: filterable list of appointments (calls `GET /api/appointments/my-appointments`). Each appointment card shows: doctor name + specialisation, appointment date + time, status chip (Pending=yellow, Confirmed=green, InProgress=blue, Completed=grey, Cancelled=red, NoShow=dark). Tap to view appointment detail screen with option to cancel (Pending only).',
    screenFile: 'lib/app/modules/student/my_appointments/my_appointments_screen.dart'
  },
  {
    file: 'Medicine Reminders Screen for Faculty and Student Feature.jpg',
    figNum: 31,
    caption: 'Figure 31: Medicine Reminders Screen — Student/Faculty View',
    role: 'Student / Faculty',
    description: 'Offline-first medicine reminder management. AppBar: "Medicine Reminders" with "+" FAB to add new reminder. Reminder list loaded from Hive local storage (`offline_medicine_reminders_{userId}` box). Each reminder card shows: medicine name, dosage, frequency, scheduled times (HH:mm), active/inactive toggle (PATCH /api/MedicineReminders/{id}/toggle). Swipe to delete. "Add Reminder" bottom sheet: medicine name, dosage, frequency (Once/Twice/Thrice/Custom daily), time picker (up to 4 times), start date, end date. Saves to Hive immediately; syncs to API when online.',
    screenFile: 'lib/app/modules/student/medicine_reminders/medicine_reminders_screen.dart'
  },
  {
    file: 'Analysis Result Screen Medi-Ai.jpg',
    figNum: 32,
    caption: 'Figure 32: AI Symptom Analysis Result Screen — Student View',
    role: 'Student',
    description: 'Structured output of the AI Symptom Analyzer (ai_symptom_result_screen.dart). Displays the structured JSON response from `POST /api/analyzer/evaluate`. Sections: (1) Possible Condition card with confidence level badge; (2) Severity indicator (Mild/Moderate/Severe/Critical) with colour coding; (3) Urgency Message; (4) Recommendations list (bulleted); (5) Home Care Guidance list; (6) "Book Appointment" CTA button if severity is Moderate or above. All AI outputs include disclaimer: "This is informational guidance only. Consult a qualified doctor for diagnosis."',
    screenFile: 'lib/app/modules/student/symptom_analyzer/ai_symptom_result_screen.dart'
  },
  {
    file: 'Admin Dashborad Screen Medi-Ai.jpg',
    figNum: 33,
    caption: 'Figure 33: Admin Dashboard — Administrator Home View',
    role: 'Administrator',
    description: 'Admin control centre (admin_dashboard_screen.dart). AppBar: "Admin Dashboard" + hamburger menu. Drawer with navigation to all admin modules. Body: (1) System statistics cards: total users, total doctors, total appointments today, pending verifications; (2) Quick action grid: Manage Users, Manage Doctors, View Appointments, Doctor Leaves, Manage Feedback, Reports, System Settings, Verifications. All data fetched from `GET /api/admin/dashboard-stats`. Source filename contains typo ("Dashborad") — corrected in caption.',
    screenFile: 'lib/app/modules/admin/dashboard/admin_dashboard_screen.dart'
  },
  {
    file: 'Doctor Dashborad Screen Medi-Ai.jpg',
    figNum: 34,
    caption: 'Figure 34: Doctor Dashboard — Doctor Role Home View',
    role: 'Doctor',
    description: 'Doctor home screen (doctor_dashboard_screen.dart). AppBar: "Doctor Dashboard" + notification bell + menu. Body: (1) Welcome card with doctor name + specialisation; (2) Today\'s appointment count badge (calls `GET /api/Doctors/appointments/today`); (3) Quick actions: Today\'s Appointments, My Patients, Set Schedule, Manage Leaves, Booking Settings, Write Prescription; (4) Upcoming appointments list. Source filename contains typo ("Dashborad") — corrected in caption.',
    screenFile: 'lib/app/modules/doctor/dashboard/doctor_dashboard_screen.dart'
  },
  {
    file: 'Set Schedule Screen by doctor.jpg',
    figNum: 35,
    caption: 'Figure 35: Set Schedule Screen — Doctor View',
    role: 'Doctor',
    description: 'Weekly availability configuration (schedule_screen.dart). AppBar: "My Schedule". Body: day-of-week cards (Mon–Sun) each with: active/inactive toggle, start time picker, end time picker. "Save Schedule" button calls `POST /api/Doctors/schedule` (upsert — creates or updates `doctorschedules` records). Only active days with valid start/end times are saved. Used by appointment booking to calculate available slots.',
    screenFile: 'lib/app/modules/doctor/schedule/schedule_screen.dart'
  },
  {
    file: 'Doctor Leave Screen.jpg',
    figNum: 36,
    caption: 'Figure 36: Doctor Leave Management Screen — Doctor View',
    role: 'Doctor',
    description: 'Leave request management (doctor_leaves_screen.dart). AppBar: "Manage Leaves" + "+" FAB. List of submitted leaves with: start date, end date, reason, status. "Add Leave" dialog: start date picker, end date picker, reason text field. Submit calls `POST /api/Doctors/leaves` (saves to `doctorleaves` table). During a leave period, the doctor\'s slots are excluded from the available-slots query.',
    screenFile: 'lib/app/modules/doctor/leaves/doctor_leaves_screen.dart'
  },
  {
    file: 'Appointment Booking Settings by doctor.jpg',
    figNum: 37,
    caption: 'Figure 37: Appointment Booking Settings Screen — Doctor View',
    role: 'Doctor',
    description: 'Booking configuration for the doctor (booking_settings_screen.dart). AppBar: "Appointment Booking Settings". Settings: slot duration (minutes per appointment — controls available-slot calculation), max concurrent bookings per slot, consultation fee display (no payment gateway — informational only). "Save Settings" calls `PUT /api/Doctors/booking-settings`.',
    screenFile: 'lib/app/modules/doctor/booking_settings/booking_settings_screen.dart'
  },
  {
    file: 'My Patients in doctor screen.jpg',
    figNum: 38,
    caption: 'Figure 38: My Patients Screen — Doctor View',
    role: 'Doctor',
    description: 'Patient list for the authenticated doctor (patients_screen.dart). AppBar: "My Patients". Displays all patients who have had at least one appointment with this doctor. Each patient card: name, last appointment date, appointment count. Tap to navigate to Patient Detail screen (patient_detail_screen.dart) showing: profile info, appointment history, medical history, prescriptions written by this doctor.',
    screenFile: 'lib/app/modules/doctor/patients/patients_screen.dart'
  },
  {
    file: 'Faculty Dashborad Screen Medi-Ai.jpg',
    figNum: 39,
    caption: 'Figure 39: Faculty Dashboard — Faculty Role Home View',
    role: 'Faculty',
    description: 'Faculty home screen (faculty_dashboard_screen.dart). AppBar: "Faculty Dashboard" + notification bell + menu. Drawer: Medicine Reminders, Profile. Body: (1) Welcome card; (2) Medicine Reminder summary (upcoming reminders count); (3) Quick actions: Medicine Reminders (only enabled action — Faculty does not have AI Symptom Analyzer or Appointment Booking). Note: Faculty module confirmed to only contain dashboard/ and medicine_reminders/ subdirectories in lib/app/modules/faculty/. Source filename contains typo ("Dashborad") — corrected in caption.',
    screenFile: 'lib/app/modules/faculty/dashboard/faculty_dashboard_screen.dart'
  },
  {
    file: 'Profile edit Screen Medi-Ai.jpg',
    figNum: 40,
    caption: 'Figure 40: Profile Edit Screen — All Roles View',
    role: 'All Roles',
    description: 'User profile management (profile_screen.dart). AppBar: "Edit Profile". Sections: (1) Profile photo with upload button (calls `POST /api/users/upload-photo`, multipart/form-data); (2) Personal info fields: Full Name, Phone Number, Gender, Address, Date of Birth; (3) Academic info: Department, Registration Number (students); (4) Doctor-specific: Specialisation, License Number, Qualification, Room Number, Bio (only shown for Doctor role); (5) "Save Changes" button calls `PUT /api/users/{id}/profile`. Uses GetX reactive state.',
    screenFile: 'lib/app/modules/common/profile/profile_screen.dart'
  },
  {
    file: 'App Logo Screen start app.jpg',
    figNum: 41,
    caption: 'Figure 41: App Logo / Onboarding Screen — Application Branding View',
    role: 'All Users (First Launch)',
    description: 'Onboarding or app logo display screen (onboarding_screen.dart). Shows the Medi-AI application logo, tagline ("Mobile-Based Healthcare Guidance and Reminder System for BUITEMS"), and a "Get Started" or "Login" call-to-action button. Displayed on first launch before the user has authenticated. Stores onboarding completion flag to SharedPreferences to skip on subsequent launches.',
    screenFile: 'lib/app/modules/auth/onboarding/onboarding_screen.dart'
  },
  {
    file: 'Mobile-based Healthcare Guidance and Reminder System for BUITEMS UI Results Medi-AI System.png',
    figNum: 42,
    caption: 'Figure 42: Medi-AI System — UI Overview Composite',
    role: 'All Roles (Composite)',
...
// ─────────────────────────────────────────────────────────────
const MISSING_SCREENSHOTS = [
  {
    figNum: 45,
    caption: 'Figure 45: Notifications Screen — All Roles View',
    toCapture: 'Open app → tap notification bell in AppBar → Notifications screen',
    screenFile: 'lib/app/modules/common/notifications/notifications_screen.dart',
    description: 'In-app notification centre (notifications_screen.dart). AppBar: "Notifications" + "Mark all read" text button. Each notification card: title, message, type icon (Appointment/Reminder/System/Health/General), time ago, read/unread indicator. Unread notifications have highlighted background. Calls `GET /api/notifications` and `PUT /api/notifications/mark-all-read`. Notification types: appointment status changes, reminder alerts, system announcements.'
  },
  {
    figNum: 46,
    caption: 'Figure 46: Admin Audit Log Screen — Administrator View',
    toCapture: 'Login as Admin → Admin Dashboard → Audit Log menu item',
    screenFile: 'lib/app/modules/admin/ (audit log view — verify route in admin controller)',
    description: 'System audit log viewer (Admin only). Displays tamper-proof log of all significant system actions from the `auditlogs` table. Each entry: action type (CREATE/UPDATE/DELETE), entity type, entity ID, old values (JSON), new values (JSON), actor (user), IP address, timestamp. Supports date-range filtering and CSV export.'
  },
  {
    figNum: 47,
    caption: 'Figure 47: Emergency Contacts Screen — All Roles View',
    toCapture: 'Login → Profile → Emergency Contacts',
    screenFile: 'lib/app/modules/common/emergency_contacts/emergency_contacts_screen.dart',
    description: 'Emergency contact management (emergency_contacts_screen.dart). AppBar: "Emergency Contacts". FAB to add contact. Each contact card: name, relationship (e.g. Father, Mother, Friend), phone number, email, "Primary" badge for isPrimary contact. Edit/delete swipe actions. Submit calls `POST/PUT /api/emergency-contacts`. Primary contact highlighted at top of list.'
  },
  {
    figNum: 48,
    caption: 'Figure 48: Doctor Reviews Screen — Student View (Post-Appointment)',
    toCapture: 'Student → My Appointments → completed appointment → "Leave Review" button',
    screenFile: 'lib/app/modules/student/my_appointments/ (review submission inline)',
    description: 'Post-appointment doctor review submission. Available only for Completed appointments. Form: star rating (1–5), written review text, anonymous toggle. Submit calls `POST /api/Doctors/{id}/reviews` (saves to `doctorreviews` table). Doctor\'s `averageRating` in `doctors` table is updated. Reviews are visible on the doctor\'s public profile.'
  },
  {
    figNum: 49,
    caption: 'Figure 49: Prescription History Screen — Student View',
    toCapture: 'Student Dashboard → Prescription History module',
    screenFile: 'lib/app/modules/student/prescription_history/prescription_history_screen.dart',
    description: 'Student prescription history (prescription_history_screen.dart). Lists all prescriptions issued by doctors for this student\'s completed appointments. Each prescription card: doctor name, appointment date, diagnosis, list of medicines (name + dosage + frequency + duration + instructions). Data from `prescriptions` + `prescriptionmedicines` tables via `GET /api/appointments/{id}/prescription`.'
  },
  {
    figNum: 50,
    caption: 'Figure 50: System Settings Screen — Administrator View',
    toCapture: 'Admin Dashboard → System Settings',
    screenFile: 'lib/app/modules/admin/system_settings/system_settings_screen.dart',
    description: 'Admin-only system configuration (system_settings_screen.dart). AppBar: "System Settings". Displays configurable key-value pairs from the `systemsettings` table. Each setting: key (e.g. MaxAppointmentsPerDay, ClinicOpenTime), current value, data type (String/Integer/Boolean/JSON), description, last updated by. Edit dialog for each setting. Changes call `PUT /api/admin/system-settings/{key}`. Settings are loaded by business logic in relevant controllers.'
  }
];

// ─────────────────────────────────────────────────────────────
// SECURITY ARCHITECTURE MERMAID (new — entirely missing from original thesis)
// ─────────────────────────────────────────────────────────────
const DIAG_SECURITY_PIPELINE = `\`\`\`mermaid
flowchart TB
...
// ─────────────────────────────────────────────────────────────
// APPOINTMENT STATE MACHINE (new activity diagram)
// ─────────────────────────────────────────────────────────────
const DIAG_APPOINTMENT_STATES = `\`\`\`mermaid
stateDiagram-v2
...
// FULL THESIS CONTENT
// ─────────────────────────────────────────────────────────────
const thesis = `# Medi-AI: Mobile-Based Healthcare Guidance and Reminder System for BUITEMS
...
**[FIGURE: ${s.file} — insert here]**

${s.caption}

*Role: ${s.role} | Source file: \`${s.screenFile}\`*
...
${MISSING_SCREENSHOTS.map(m => `
**[SCREENSHOT NEEDED — ${m.caption}]**

*How to capture:* ${m.toCapture}
*Screen file:* \`${m.screenFile}\`
...
// WRITE OUTPUT
// ─────────────────────────────────────────────────────────────
const outPath = 'C:/D/FYP/Medi-AI_F-B-main/Medi_AI_Backend_railway/thesis/Medi-AI - Final (Updated).md';
fs.writeFileSync(outPath, thesis, 'utf8');

const stats = fs.statSync(outPath);
console.log('=== Medi-AI Thesis Generator v2 ===');
console.log('Output:', outPath);
console.log('Size:', Math.round(stats.size / 1024), 'KB');
console.log('Lines:', thesis.split('\n').length);
console.log('');
console.log('STEP 0 SUMMARY:');
console.log('  - .docx: Read via ZIP parser ✅');
console.log('  - write_thesis.js: Extended (v1 → v2) ✅');
console.log('  - 7 Mermaid diagrams: All read and embedded ✅ (2 corrected for wrong routes/Faculty scope)');
console.log('  - 21 screenshots: All inventoried with code-verified captions ✅');
console.log('  - Unreadable files: NONE');
console.log('');
console.log('KEY CORRECTIONS (Phase 1):');
console.log('  - SmarterASP.net → Railway everywhere');
console.log('  - /api/AI/analyze → POST /api/analyzer/evaluate');
console.log('  - /api/reminders → POST /api/MedicineReminders');
console.log('  - "GroqCloud Llama 3" → "Groq Cloud API (llama-3.1-8b-instant)"');
console.log('  - Faculty: removed AI/Booking from use case and nav diagram');
console.log('  - ERD: expanded notation to 23 entities');
console.log('  - Email domain: corrected to client-side-only claim');
console.log('');
console.log('NEW SECTIONS ADDED:');
console.log('  - Security Implementation subsection (rate limiting + JWT revocation)');
console.log('  - Appointment state machine activity diagram (6 states)');
console.log('  - Security pipeline Mermaid diagram');
console.log('  - 4 code snippets with explanations');
console.log('  - 23-case formal test case table reference');
console.log('  - Objectives achievement mapping (Table 8)');
console.log('  - Related systems comparison (Table 6)');
console.log('  - Technology justification table (Table 7)');
console.log('  - Limitations section (7 items)');
console.log('');
console.log('HUMAN ACTION REQUIRED:');
console.log('  HAR1 (HIGH): Capture 6 missing screenshots (Figures 45-50)');
console.log('  HAR2 (HIGH): Attach UAT survey data');
console.log('  HAR3 (HIGH): Attach Postman collection export');
console.log('  HAR4 (HIGH): Ethics approval confirmation');
console.log('  HAR5 (MEDIUM): Generate 23-entity ERD PNG');
console.log('  HAR6 (MEDIUM): Render 7 Mermaid diagrams to PNG');
console.log('  HAR7-8 (MEDIUM): Regenerate TOC + List of Figures with page numbers');
